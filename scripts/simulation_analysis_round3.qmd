...
## Known propensities, no $\pi(\hat{y})$ projection, no residualization, $\hat{y}$ as XBCF covariates, GRF using regularized margins
...
## Estimated propensities, no $\pi(\hat{y})$ projection, no residualization, $\hat{y}$ as XBCF covariates, GRF using regularized margins
...
## Known propensities, no $\pi(\hat{y})$ projection, residualization, $\hat{y}$ as XBCF covariates, GRF using regularized margins

```{r, echo=F}
rmse_vis_df <- rmse_df %>% 
    ungroup() %>%  
    filter((`Estimated Propensities`==0) & (`Project Pi Yhat`==0) & (`Residualized XBCF` == 1)) %>% 
    select(
        `DGP Number`, `Sample Size`, `Number of Variables`, 
        `Noise to Signal Ratio`, `XBCF RMSE`, 
        `Multiple Propensity XBCF RMSE`, `GRF RMSE`, 
...
## Estimated propensities, no $\pi(\hat{y})$ projection, residualization, $\hat{y}$ as XBCF covariates, GRF using regularized margins

```{r, echo=F}
rmse_vis_df <- rmse_df %>% 
    ungroup() %>%  
    filter((`Estimated Propensities`==1) & (`Project Pi Yhat`==0) & (`Residualized XBCF` == 1)) %>% 
    select(
        `DGP Number`, `Sample Size`, `Number of Variables`, 
        `Noise to Signal Ratio`, `XBCF RMSE`, 
        `Multiple Propensity XBCF RMSE`, `GRF RMSE`, 
...
## Estimated propensities, no $\pi(\hat{y})$ projection, no residualization, no $\hat{y}$ XBCF as covariates, GRF using regularized margins
...
## Estimated propensities, no $\pi(\hat{y})$ projection, residualization, no $\hat{y}$ XBCF as covariates, GRF using regularized margins
